#ifndef GENOMEMAPPER_SRC_PARSER_H_
#define GENOMEMAPPER_SRC_PARSER_H_
...
class Parser {  // Parser factory
  ...
  protected:
    Parser(gzFile file, std::uint32_t storage_size)
            : file_(file, gzclose),
              buffer_(65536,
                      0),  // 64 kB // actually may not be necessary to use fill constructor to set them to default 0
              buffer_ptr_(0),
              buffer_bytes_(0),
    ...
    [[nodiscard]] std::uint32_t buffer_ptr() const {
        return buffer_ptr_;
    }
    ...
    [[nodiscard]] const std::vector<char> &storage() const {
        return storage_;
    }

    [[nodiscard]] std::uint32_t storage_ptr() const {
        return storage_ptr_;
    }
...
#endif //GENOMEMAPPER_SRC_PARSER_H_
